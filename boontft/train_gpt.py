...
Combines best proven techniques with EMA (not SWA) and test-time training.
SDPA fallback for consumer GPU development (4090).
"""
...
class Hyperparameters:
    ...
    # EMA (replaces SWA — proven 0.003 bpb better)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        v = self.c_v(x)
        # Value embedding: add token identity directly to values before reshape
...
class GPT(nn.Module):
    def __init__(
        ...
        # Set partial RoPE on all attention blocks
        if rope_dims > 0:
            head_dim = model_dim // num_heads
            for block in self.blocks:
                block.attn.rope_dims = rope_dims
                block.attn.rotary = Rotary(head_dim, base=rope_base, train_seq_len=1024, rope_dims=rope_dims)
        # Value embeddings: 1 SHARED table, per-layer learned scales
    ...
    def _get_ve(self, layer_idx: int, input_ids: Tensor, ve_cache: dict | None = None) -> Tensor | None:
        ...
        if self.ve_shared is None or layer_idx not in self.ve_layer_indices:
            return None
        # Cache the shared VE computation (same for all layers, different scale)
...
def ttt_adapt(
    ...
    """Adapt model on validation data via SGD before final eval."""
    ...
    for epoch in range(args.ttt_epochs):
        ...
        if log_fn:
            ...
            log_fn(f"ttt_epoch:{epoch+1}/{args.ttt_epochs} "
                   f"loss:{epoch_loss.item()/max(epoch_tokens.item(),1):.4f} time:{elapsed:.1f}s")
...
def main() -> None:
    ...
    while True:
        ...
        if args.swa_enabled and not args.ema_enabled and scale < 0.2 and step % args.swa_every == 0:
        ...
        # EMA update (every step, on GPU for speed)
    ...
    if args.swa_enabled and swa_state is not None and swa_count > 1:
        ...
        # Diagnostic eval: measure quality after SWA, before quantization
        torch.cuda.synchronize()
    ...
    # Apply EMA weights (replaces SWA for better quality)
    if ema_state is not None:
    ...
    # TTT: adapt model on validation data before sliding eval
    if args.ttt_enabled:
        if distributed:
            dist.barrier()
